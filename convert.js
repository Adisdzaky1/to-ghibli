...
const colorifyai = {
  ...
  async uploadImage(imagePath) {
    ...
    const response = await axios.post(`${this.baseUrl}/aitools/upload-img`, data, {
      headers: {
        ...this.baseHeaders,
  ...
  async createTask(imagePath, prompt, useGhibliStyle) {
    const lora = useGhibliStyle ? ["ghibli_style_offset:0.8"] : [];
    const response = await axios.post(`${this.baseUrl}/aitools/of/create`, {
  ...
  async checkStatus(taskId) {
    const response = await axios.post(`${this.baseUrl}/aitools/of/check-status`, {
      task_id: taskId,
      fn_name: "demo-auto-coloring",
      call_type: 3,
      request_from: 10,
...
export default async (req, res) => {
  ...
  try {
    ...
    const prompt = req.headers['x-prompt'] || "(masterpiece), best quality";
    const useGhibliStyle = req.headers['x-style'] === 'ghibli';
